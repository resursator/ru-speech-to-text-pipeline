...
# ASR health check
# ---------------------------------------------------------------------------
ASR_HEALTH_TIMEOUT  = int(os.getenv("ASR_HEALTH_TIMEOUT",  str(15 * 60)))
ASR_HEALTH_INTERVAL = int(os.getenv("ASR_HEALTH_INTERVAL", "10"))


def _wait_for_asr(timeout: int = ASR_HEALTH_TIMEOUT, interval: int = ASR_HEALTH_INTERVAL) -> None:
    """
    Блокирует выполнение до тех пор, пока ASR-сервис не вернёт
    {"status": "ready"} или {"status": "ok"} на GET /health.
    """
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if resp.status_code == 200 and asr_status in ("ok", "ready"):
...
def ffmpeg_convert(self, task_id: str, input_path: str, callback_url: str = ""):
    ...
    _send_callback(callback_url, task_id, "converting")
...
def denoise(self, prev: Dict[str, Any]):
    ...
    _send_callback(callback_url, task_id, "denoising")
...
def transcribe(self, prev: Dict[str, Any]):
    ...
    _send_callback(callback_url, task_id, "waiting_for_asr")
    ...
    _send_callback(callback_url, task_id, "transcribing")
...
def process_audio(task_id: str, file_path: str, callback_url: str = "") -> Dict[str, Any]:
    ...
    _send_callback(callback_url, task_id, "queued")
